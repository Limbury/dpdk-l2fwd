	...
	if(iphdr->type_of_service == SLOW_FLOW){//Packet dropped due to queue 
		ret = rte_ring_sp_enqueue(
             		app.rings_rx[src_port],
               		pkt);
		if(!ret){ // add the pointer to addr_queue
			ret = rte_ring_sp_enqueue(
		...
		if(front == NULL){
			...
			(*front) = (*pkt); //replace
			ret = rte_ring_mc_dequeue(app.addr_rings_rx[src_port],
						(void **)&front
						);
			if(!ret)
					front = NULL;	
			//coming pkt is fast flow
...
/*	if(free_buffer < (app.ring_rx_size>>2) ){ //don't have enough free space
		if(iphdr->type_of_service == SLOW_FLOW){//Packet dropped due to queue 
			rte_pktmbuf_free(pkt);		//don't have enough free space
			RTE_LOG(
		            DEBUG, SWITCH,
